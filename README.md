...
This repository is step 1 of 3: the production page supports searching, loading, refreshing, paging and reading threads. Wallet send/reply and the complete interaction layer are scheduled for step 2.
...
`dist/` is committed and can be published directly to IPFS or any static host. Vite uses `base: './'`, hash routes, and relative built assets, so an HTTP rewrite is unnecessary.
...
- The browser keeps unread baselines, opened threads and accepted requests in local storage, with a non-throwing memory fallback. There is no server, analytics, cookie or external script/font.
...
- `npm run ci` — passed after final changes; details include the exact test and gzip totals printed by the command.
...
Frontend review used the [Vercel Web Interface Guidelines](https://raw.githubusercontent.com/vercel-labs/web-interface-guidelines/main/command.md), retrieved 2026-09-25. Reviewed `index.html`, `src/App.tsx`, `src/styles.css` and `src/review.css`.

- `src/App.tsx:17`: added a keyboard skip link and main landmark target.
- `src/App.tsx:19`: the search has a linked label, name, autocomplete hint, inline error and example placeholder.
...
Automated interaction tests cover the pinned board, search control and unknown route. No browser automation or screenshot tooling was available, so real-browser console/resource inspection and visual comparison at mobile/desktop widths remain unperformed.
